...
public class MoveableGameObject extends GameObject {
    ...
    public MoveableGameObject(DemolitionWars game){
	...
	private void move() {
		if (speed > 0 || moveX != 0 || moveY != 0) {
            ...
            if(this instanceof Obtainables || !isGonnaCollide(xlocation+movementX, ylocation+movementY)) {
                doGunActions(xlocation+movementX, ylocation+movementY);
                xlocation += movementX;
                ylocation += movementY;
                if(this instanceof Obtainables){
                    MovingObject thisObject = (MovingObject)this;
                    thisObject.inAir=3;
                }
            } else {
                hasCollided=true;
    ...
    private boolean isGonnaCollide(double x, double y){
        Human thisHuman = (Human)this;
        for(MovingObject block: thisHuman.nearbyObjects){
            if(block instanceof Block && !game.imageLoader.hasNoCollision(block)){
                if(x>block.getX()-(2*64) && y>block.getY()-(1.5*64) && x<block.getX()+(2*64) && y<block.getY()+(1.5*64)){
                    if(block instanceof BlockDoor && this instanceof Player){
                        BlockDoor door = (BlockDoor)block;
                        if (game.world.humans.get(0).team == door.team) {
                            door.open();
                            return false;
                        } else {
                            return true;
                    ...
                    return true;
                } else if(x>block.getX()-(2*64) && y+(game.tileSize*4)>block.getY()-(1.5*64) && x<block.getX()+(2*64) && y+(game.tileSize*4)<block.getY()+(2*64)){
                    return true;
                }
            }
        }
    ...
    public void doGunActions(double x, double y){
        if(this instanceof ItemBullet){
            for(int i =0; i<game.world.blocks.size(); ){
                    if(x>game.world.blocks.get(i).getX()-(2*64) && y>game.world.blocks.get(i).getY()-(2*64) && ((x<game.world.blocks.get(i).getX()+(4*64) && y<game.world.blocks.get(i).getY()+(4*64) && game.world.blocks.get(i) instanceof Human && ((ItemBullet)this).shotByPlayer) || (x<game.world.blocks.get(i).getX()+(2*64) && y<game.world.blocks.get(i).getY()+(2*64) && game.world.blocks.get(i) instanceof Block))){
                        game.world.blocks.get(i).die();
                        ((MovingObject)this).die();
            ...
            if(x>game.world.humans.get(0).getX()-(2*64) && y>game.world.humans.get(0).getY()-(2*64) && x<game.world.humans.get(0).getX()+(4*64) && y<game.world.humans.get(0).getY()+(4*64) && !((ItemBullet)this).shotByPlayer){
                game.world.humans.get(0).health-=100;
                ((MovingObject)this).die();
            } else if(x<0 || y<0 || x>game.getMapWidth() || y>game.getMapHeight()){
                ((MovingObject)this).die();
            }
        }
    }
