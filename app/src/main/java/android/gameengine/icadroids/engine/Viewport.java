...
public class Viewport
{
	...
	void update() {
        ...
        /*
        As the gameengine didn't fit all my needs, I made my own way of loading the viewport.
         */
        if (player != null) {
            com.diygames.Humans.Human humanPlayer = (com.diygames.Humans.Human)player;
            com.diygames.demolitionwars.DemolitionWars gameInstance = humanPlayer.game;
            
            // Calculate viewport boundaries with scaling factor
            int viewportWidth = (int)((1920 * gameInstance.scaleFactorX) / gameInstance.spriteScaleFactor);
            int viewportHeight = (int)((1080 * gameInstance.scaleFactorY) / gameInstance.spriteScaleFactor);
            
            if (player.getX() > viewportWidth / 2 && player.getX() < player.game.getMapWidth()-(viewportWidth / 3)) {
                viewportX = player.getX() - viewportWidth / 2;
            }
            if (player.getY() > viewportHeight / 3 && player.getY() < player.game.getMapHeight()-(viewportHeight / 3)) {
                viewportY = player.getY() - viewportHeight / 3;
            }
        }
	}
